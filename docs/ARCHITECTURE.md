...
1. **The UI has not been compiled.** This repo was built on Linux, where no iOS
   ...
   (136 + 152 + 36 tests); every Swift file passes `swiftc -parse`; the `.xcodeproj` is
...
| Web core | `web/src/core/*.test.ts` | 152 | The same, ported, plus the persisted-state reward rules and a JSON round trip |
