...
Xcode. That's how the 136 Swift tests in this repo were written and run.

## The web client

```
┌────────────────────────────────────────────────────┐
│ features/         React screens, one per stage     │
│ services/         Recorder, transcription, voice,  │
│                   coaching client                  │
│ app/store         localStorage persistence         │
├────────────────────────────────────────────────────┤
│ core/             Pure TypeScript. No DOM, no      │
│                   network. Unit-tested             │
│ content/          content.json ← generated from    │
│                   the Swift definitions            │
└────────────────────────────────────────────────────┘
```

Same shape, same boundary. `web/src/core` is a faithful port of `Core`: the
metrics algorithms, the tolerant decoder, quote grounding, prompt construction,
the offline coach, XP, mastery gating and spaced review. Its test suite mirrors
the Swift one case for case, so a divergence shows up as a failing test rather
than as two apps that disagree about how many filler words you said.

### Why the content is generated rather than ported

Algorithms are worth porting: they are a few hundred lines and their behaviour
is pinned by tests on both sides. Content is not — twenty-four scenarios and
thirty-eight skills would drift on the first edit, silently, in whichever copy
you forgot. So the Swift definitions stay authoritative, `Tools/ContentExport`
encodes them to JSON (every content type is already `Codable`), and CI
...
| | iOS | Web |
| --- | --- | --- |
| Persistence | SwiftData | `localStorage`, one JSON blob |
| Recording | `AVAudioRecorder` with metering | `MediaRecorder` + `AnalyserNode` |
| Transcription | Apple Speech, on-device where supported | Web Speech API, or typed |
| Word timings | Recogniser segment timestamps | Distributed across measured speech regions |
| Character voice | `AVSpeechSynthesizer` | `speechSynthesis` |
| Streak calendar | `Calendar.current` | ISO weeks, so it is locale-independent |

The word-timing difference is the one that matters. The browser's speech
recognition returns no per-word timestamps, so the web build samples the
microphone signal twenty times a second, derives the speech and silence regions
from the energy envelope, and distributes the transcript's words across them.

That means **pauses, leading silence and speaking time are measured** — from the
audio, more directly than the iOS build gets them — while **where a word sits
inside a phrase is an estimate**. Both facts are stated on the metrics panel and
in the prompt sent to the model, because the alternative is feedback that quietly
implies more precision than exists.

### The web app never fails to give feedback either

A page served from static hosting has no server of its own, so "no coaching
service configured" is the *normal* state, not an error state. Every coaching
path falls back to the same rule-based engine as the iOS build, and the UI says
which one you got. Recording, measurement, the full training loop, progression,
spaced review and the curriculum all work with nothing configured at all.
...
1. **The UI has not been compiled.** This repo was built on Linux, where no iOS
   SDK exists. Core, the web client and the proxy are genuinely built and tested
   (136 + 151 + 36 tests); every Swift file passes `swiftc -parse`; the `.xcodeproj` is
...
9. **Browser speech recognition is uneven.** Chrome and Safari implement the Web
   Speech API; Firefox does not. Where it is missing or returns nothing, the web
   app measures the recording as normal and asks the learner to type what they
   said, so the loop always completes — but the transcript is then typed rather
   than heard. Safari's implementation also sends audio to Apple to transcribe;
   that is the browser's own feature, and the app says so in Settings.
10. **Web audio is never persisted.** Recordings live in the tab and are
    discarded on leaving the session, so the web build has no "keep my
    recordings" option at all. Simpler, and a stronger privacy default — but it
    means you cannot compare last week's audio to today's.
11. **The web app has not been run on a physical iPhone.** It was built and
    exercised headlessly in Chromium. Microphone permissions and speech
    recognition on iOS Safari specifically need a real device to confirm.

---

## Suggested next steps

**Before anything else:** publish the web app, open it on the phone, and run one
session end to end. That is now the shortest path to using this thing — the iOS
build still needs a Mac, Xcode and a signing identity.

Then, roughly in order of value:

1. **Play the recording back on the iOS feedback screen**, seeking to the quoted
   moment. `TranscriptGrounding.locate` already exists, and the web build
   already offers playback — this is UI work only, and it is the single biggest
   addition to how the feedback lands.
...
| --- | --- | --- | --- |
| Core | `Tests/SpeakLabCoreTests` | 136 | Metrics, question classification, decoding, grounding, XP, levels, mastery gating, review scheduling, streaks, content integrity, offline coach |
| Web core | `web/src/core/*.test.ts` | 151 | The same, ported, plus the persisted-state reward rules and a JSON round trip |
| Proxy | `server/src/test` | 36 | Validation, auth, rate limiting, schema enforcement, refusal, truncation, upstream error mapping, fallback degradation, CORS allowlisting |
| Syntax | `Tools/parse_check.sh` | 50 files | Every Swift file parses |

All four run on every push via `.github/workflows/ci.yml`, which also
regenerates `content.json` and fails if the committed copy is stale.
